...
class sgwindow_portfolio extends WP_Widget {
	...
	*/
	public function __construct() {
		...
		'description' => __('Display Portfolio Projects.', 'cats456' ));
		...
		/* Create the widget. */		
		parent::__construct( 'sgwindow_portfolio_widget', __( 'SG Portfolio Projects (SG Window)', 'cats456' ), $widget_ops, $control_ops );
			...
			<div class="main-wrapper-image <?php echo $pos_class;?>" style="padding:<?php echo esc_attr($instance['padding_top']);?>px <?php echo esc_attr($instance['padding_right']);?>% <?php echo esc_attr($instance['padding_bottom'])?>px <?php echo esc_attr($instance['padding_left'])?>%">
				...
				<div class="description">
					<article>
						...
						<a href="<?php echo esc_url( get_term_link( $instance['project'], $tax ) ); ?>" class="link" rel="bookmark"><?php _e('All Projects', 'cats456'); ?></a>
					</article> <!-- article -->
				</div> <!-- .description -->
				...
				<div class="wrapper-image <?php echo esc_attr($instance['columns']).( $instance['is_step'] ? ' step' : ' all' ).( $instance['is_hover_all'] ? ' hover-all' : '' ).( $instance['is_margin_0'] ? ' margin-0' : '' );?>">
					...
					<div class="element <?php echo esc_attr($instance['effect_id']).( $instance['is_animate_once'] ? ' once' : '' ).( $instance['is_animate'] ? ' animate' : '' );?>">
						<article>
							...
							<div class="entry-thumbnail" style="background-image:url('<?php echo esc_url( $image[0] ); ?>')">																
							</div>
	...
	*/
	function form( $instance ) {
		...
		sgwindow_echo_input_text( $this, 'title', $instance, __( 'Title: ', 'cats456' ), 0);
		...
		if ( $terms && ! is_wp_error( $terms ) ) : 
			...
			esc_html_e('Project:', 'cats456'); ?>
			<select id="<?php echo $this->get_field_id('project'); ?>" name="<?php echo esc_attr( $this->get_field_name('project') ); ?>" style="width:100%;">
				<option value="0"><?php esc_html_e('Any', 'cats456'); ?></option>
			<?php 
		...
		<?php 
		esc_html_e('Image Size:', 'cats456'); ?>
		<select id="<?php echo $this->get_field_id('image_size'); ?>" name="<?php echo esc_attr( $this->get_field_name('image_size') ); ?>" style="width:100%;">
		<?php 
			$styles=array( __('Post Thumbnail', 'cats456'), __('Thumbnail', 'cats456'), __('Large', 'cats456'), __('Full', 'cats456'));
		...
		sgwindow_echo_input_checkbox( $this, 'is_step', $instance, __( 'Step Animation', 'cats456'));
		sgwindow_echo_input_checkbox( $this, 'is_margin_0', $instance, __( 'No Margins', 'cats456'));

		sgwindow_echo_input_hover_style( $this, 'effect_id', $instance);

		sgwindow_echo_input_checkbox( $this, 'is_animate', $instance, __( 'Animate', 'cats456'));
		sgwindow_echo_input_checkbox( $this, 'is_animate_once', $instance, __( 'Once', 'cats456'));
		
		sgwindow_echo_input_text( $this, 'count', $instance, __( 'Count: ', 'cats456' ), 0);
	...
	function defaults( $instance ){
		...
		$defaults = array('title' => '',
						  ...
						  'link_caption'   => __('More Info', 'cats456'),
						  'padding_right'   => '0',
						  'padding_left'   => '0',
						  'padding_top'   => '0',
						  'padding_bottom'   => '0',
