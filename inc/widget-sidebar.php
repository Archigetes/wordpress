...
class sgwindow_side_bar extends WP_Widget {
	...
	*/
	public function __construct() {
		...
		'description' => __( 'Allows to build a page layout by adding new sidebars with custom number of columns and column width.', 'cats456' ));
		...
		parent::__construct( 'sgwindow_side_bar', __( 'SG cats456', 'cats456' ), $widget_ops, $control_ops );
	...
	function widget( $args, $instance ) {
		...
		for( $i = 0; $i < 4; $i++ ) {
			...
			if ( ! isset( $instance[ 'sidebar_name_' . $i ] ) ) {
				$instance[ 'sidebar_name_' . $i ] = __( 'Sidebar', 'cats456' ) . ' ' . ( $i + 1 );
			}
					...
					<?php else : ?>
						...
						<div style="text-align: center;padding: 30% 20%;">
							<a style="color: #000;" href="<?php echo esc_html( home_url() .  '/wp-admin/customize.php?autofocus[panel]=widgets' ); ?>">
								<?php echo __( 'Place for your Content, Sidebar', 'cats456' ) . ' ' . esc_attr( $instance[ 'sidebar_id_' . $i ] ); ?> 
							</a>
						</div>
	...
	*/
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'count' ) );?>"><?php echo esc_html( __( 'Number of sidebars (number in range [1:4]) (Press Apply): ', 'cats456' ) ); ?></label>
			<br>
		...
		<span id="<?php echo $this->get_field_id( 'sidebar_id_0' ); ?>_b" class="add-sidebar-button <?php echo esc_attr( $instance['count'] ). ' ' . $widget_id; ?> <?php echo ( ! isset( $instance[ 'sidebar_id_0' ] ) ? 'active-class' : '' ); ?>">
			<?php _e( 'Register New Sidebars', 'cats456' ); ?>
		...
		<?php
			...
			for( $i = 0; $i < 4; $i++ ) {
				...
				if ( ! isset( $instance[ 'sidebar_name_' . $i ] ) || empty ( $instance[ 'sidebar_name_' . $i ] ) ) {
					$instance[ 'sidebar_name_' . $i ] = __( 'Sidebar', 'cats456' ) . ' ' . ( $id );
				}
				...
				<div id="<?php echo esc_attr( $this->get_field_id( 'sidebar_id_' . $i ) ); ?>_hide" class="sidebar-button" <?php echo ( $i >= $instance['count'] ? 'style="display:none"' : '' ); ?>>
					<?php
					sgwindow_echo_section_main_start( __( 'Edit Sidebar ', 'cats456' ) . ( $i + 1 ), $this->get_field_id( 'width_' . $i ) . $i );

						sgwindow_echo_input_text( $this, 'sidebar_name_' . $i, $instance, __( 'Name of sidebar (You will see this name after refreshing Customizer): ', 'cats456' ) );
						sgwindow_echo_input_text( $this, 'width_' . $i, $instance, __( 'Width of sidebar (%): ', 'cats456' ) );
...
function sgwindow_side_bar_reg( $count ){
	...
	if ( class_exists ( 'WP_Customize_Manager' ) ) {
		...
		for( $i = $sgwindow_widget_sidebars; $i < $sgwindow_widget_sidebars + $count; $i++ ) {
			register_sidebar( array(
				'name' => __( 'Sidebar', 'cats456' ) . ' ' . absint( $i ),
				'id' => 's_' . $i,
				'before_widget' => '<aside id="%1$s" class="widget %2$s">',
				'after_widget' => "</aside>",
				'before_title' => '<h3 class="widget-title">',
