...
define( 'SGWINDOWCHILD', 'SGCats456' );
  
function cats456_setup() {

	$defaults = sgwindow_get_defaults();

	load_child_theme_textdomain( 'cats456', get_stylesheet_directory() . '/languages' );
...
add_action( 'after_setup_theme', 'cats456_setup' );
...
function cats456_setup_colors() {
	...
	$sgwindow_colors_class->add_color($i, $section_id, __('Link', 'cats456'), $p++, false);
	...
	$sgwindow_colors_class->add_color($i, $section_id, __('H1-H6 heading', 'cats456'), $p++, false);
	...
	$sgwindow_colors_class->add_color($i, $section_id, __('H1-H6 Link', 'cats456'), $p++, false);
	$sgwindow_colors_class->set_color($i, 0, '#000000');	
	$sgwindow_colors_class->set_color($i, 1, '#000000');	
	$sgwindow_colors_class->set_color($i, 2, '#1e73be');
	...
	$sgwindow_colors_class->add_color($i, $section_id, __('Description', 'cats456'), $p++, false);
	...
	$sgwindow_colors_class->add_color($i, $section_id, __('Link Hover', 'cats456'), $p++, false, 'refresh');
	$sgwindow_colors_class->set_color($i, 0, '#000000');
	$sgwindow_colors_class->set_color($i, 1, '#000000');
	$sgwindow_colors_class->set_color($i, 2, '#000000');
}
add_action( 'after_setup_theme', 'cats456_setup_colors', 100 );
...
function cats456_styles() {
    wp_enqueue_style( 'sgwindow-style', get_template_directory_uri() . '/style.css' );
    wp_enqueue_style( 'cats456-style', get_stylesheet_uri(), array( 'sgwindow-style' ) );
	
	wp_enqueue_style( 'cats456-colors', get_stylesheet_directory_uri() . '/css/scheme-' . esc_attr( sgwindow_get_theme_mod( 'color_scheme' ) ) . '.css', array( 'sgwindow-style', 'sgwindow-colors' ) );
...
add_action( 'wp_enqueue_scripts', 'cats456_styles' );
...
function cats456_defaults( $defaults ) {
	...
	$defaults['header_font'] = 'Josefin Slab';
	...
	$defaults['size_image_height'] = '700';
	...
	$defaults['footer_text'] = '<a href="' . __( 'http://wordpress.org/', 'cats456' ) . '">' . __( 'Powered by WordPress', 'cats456' ). '</a> | ' . __( 'theme ', 'cats456' ) . '<a href="' .  __( 'http://wpblogs.ru/themes/', 'cats456') . '">cats456</a>';
	
	return $defaults;

}
add_filter( 'sgwindow_option_defaults', 'cats456_defaults' );

/** Set theme layout
 *
 * @since cats456 1.0
 */
function cats456_layout( $layout ) {
...
add_filter( 'sgwindow_layout', 'cats456_layout' );

/**
 * Hook widgets into left sidebar at the front page
 *
 * @package WordPress
 * @subpackage cats456
 * @since cats456 1.0
...
function cats456_home_left_column( $layouts ) {
...
add_action('sgwindow_empty_column_1-home', 'cats456_home_left_column', 20);
...
function cats456_home_right_column( $layouts ) {
...
add_action('sgwindow_empty_column_2-home', 'cats456_home_right_column', 20);
...
function cats456_home_footer_1() {

	the_widget( 'WP_Widget_Categories' );
	the_widget( 'WP_Widget_Search', 'title=' . __( 'Search', 'cats456' ) );

}
add_action('sgwindow_empty_sidebar_footer-1', 'cats456_home_footer_1', 20);
...
function cats456_home_footer_2() {
...
add_action('sgwindow_empty_sidebar_footer-2', 'cats456_home_footer_2', 20);
...
function cats456_home_footer_3() {

	the_widget( 'WP_Widget_Calendar' );
	the_widget( 'WP_Widget_Archives' );

}
add_action('sgwindow_empty_sidebar_footer-3', 'cats456_home_footer_3', 20);
...
function cats456_header( $layouts ) {
			...
			<?php if ( get_header_image() 
				...
				<div class="sg-site-header-1 my-image widget">
					<?php if ( '1' == sgwindow_get_theme_mod( 'is_parallax_header' ) ) : ?>
						...
						<div class="parallax-image 0 0 35 <?php echo esc_attr( sgwindow_get_theme_mod( 'parallax_image_speed' ) ); ?> 0 0" style="background-image: url(<?php header_image(); ?>);">
						...
						<?php if ( '' != sgwindow_get_theme_mod( 'logotype_url' ) ) : ?>
							<div class="logo-block">
								<a class="logo-section" href='<?php echo esc_url( home_url( '/' ) ); ?>' title='<?php echo esc_attr( get_bloginfo( 'name', 'display' ) ); ?>' rel='home'>
									<img src='<?php echo esc_url( sgwindow_get_theme_mod( 'logotype_url' ) ); ?>' class="logo header-hovered-logo" alt='<?php echo esc_attr( get_bloginfo( 'name', 'display' ) ); ?>'>
								</a><!-- .logo-section -->
							</div><!-- .logo-block -->
						<?php endif; ?>
...
add_action('sgwindow_header_menu', 'cats456_header', 20);
 ...
 * @since cats456 1.0
 */
function cats456_right_sidebar_portfolio() {

	the_widget( 'sgwindow_portfolio', 'title='.__('Recent Projects', 'cats456').
								'&count=8'.
...
add_action('sgwindow_empty_column_2-portfolio-page', 'cats456_right_sidebar_portfolio', 20);
...
add_action('wp_head', 'cats456_hook_css');
 ...
 * @since cats456 1.0.0
*/
add_action( 'customize_register', 'cats456_create_controls', 999 );
function cats456_create_controls( $wp_customize ) {
 ...
 * @since cats456 1.0.0
 */
function cats456_customize_preview_js() {
	wp_enqueue_script( 'cats456-customizer', get_stylesheet_directory_uri() . '/js/theme-customizer.js', array( 'customize-preview' ), time() . '11.12.2020', true );
}
add_action( 'customize_preview_init', 'cats456_customize_preview_js', 99 );
